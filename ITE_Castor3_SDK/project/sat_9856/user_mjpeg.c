...
static pthread_t tidDeMjpeg, tidDeAudio, tidAVIOpen;
 bool      g_videoPlayerIsFileEOF = false;
    ...
    void *arg)
{
	...
	while(!g_videoPlayerIsFileEOF )
	{
		while(pCur &&(!g_videoPlayerIsFileEOF ))
		{
			FourCC = fourChar(*pCur,*(pCur+1) ,*(pCur+2),*(pCur+3));
		...
		while( (int_count != dec_count) && --timeout_ms &&(!g_videoPlayerIsFileEOF ))
		{
			usleep(1000*10);
		}
	...
	if(DA_free >= len)
	{
		if((DA_w + len) >= DAC_BUFFER_SIZE)
		{
			...
			if(szbuf>0)
			{
				...
				ithFlushDCacheRange(dac_buf  + DA_w, szbuf);
			}
			DA_w = len - szbuf;
			memcpy(dac_buf , data+szbuf, DA_w);
			ithFlushDCacheRange(dac_buf , DA_w);
		}
		else
		{
...
{
	initDA();
	i2s_set_direct_volperc(Volume_level);
...
void *_user_video_play(void *arg)
{
	//VideoInit();
	printf("_user_video_play1111111111111111111111\n");
	...
	printf("_user_video_play2222222222222222222\n");
	pthread_create(&tidAVIOpen, NULL, AVIFileOpen, (void *)"E:/rec.avi");
	usleep(1000*100);

	//create jpeg decode thread for jpeg decode.
	pthread_create(&tidDeMjpeg, NULL, _DeMjpegThread,  NULL);
	pthread_create(&tidDeAudio, NULL, _DeAudioThread,  NULL);
	printf("_user_video_play3333333333333333333\n");
#if 0
	pthread_t play_over;		
	pthread_create(&play_over, NULL, __wait_play_over,  NULL);
	
	while(1)
	{
		if(PlayerStop||((!tidDeMjpeg)&&(!tidAVIOpen)))
		{ 
...
void *user_video_play(void *arg)
{
	pthread_t play_p;			//Ïß³ÌID
	_user_video_play(arg);
	//pthread_create(&play_p,NULL, _user_video_play, NULL);
	//pthread_detach(play_p);
}

void __WaitPlayMjpegFinish(void)
{
    if(tidDeMjpeg)
	{
