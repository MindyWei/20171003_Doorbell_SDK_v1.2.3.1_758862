...
static bool                       audioKeySound2Play;
...
void AudioPlayKeySound(void)
{
    ...
    if(audioKeySound2Play == true)
        audioKeySound2Play = false;
}

void AudioPlayKeySound2(void)
{
    char filepath[PATH_MAX];
    audioKeySoundPaused = true;
    
    if(audioKeySound2Play == true && smtkAudioMgrGetState() != SMTK_AUDIO_PLAY){
        //smtkSetQuickPlayFlag();
        audioKeySound2Play = true;
        audioKeySoundPaused = false;
        return;
    }
    
    if (theConfig.keylevel > 0 && smtkAudioMgrGetState() != SMTK_AUDIO_PLAY && audioKeySound2Play == false)
    {
        bool muted = AudioIsMuted();
        
        smtkAudioMgrSetVolume(theConfig.keylevel);

        strcpy(filepath, CFG_PRIVATE_DRIVE ":/sounds/key11.wav");
        //strcat(filepath, theConfig.keysound);
