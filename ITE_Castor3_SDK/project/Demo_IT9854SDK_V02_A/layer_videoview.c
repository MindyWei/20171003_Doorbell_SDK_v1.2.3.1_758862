...
static ITUTrackBar* videoViewTrackBar;
...
bool VideoViewOnTimer(ITUWidget* widget, char* param)
{
    ...
    if (videoPlayerIsPlaying)
    {
        ...
        ituTrackBarSetValue(videoViewTrackBar, videoPlayerPercentage);
		//my.wei add
...
bool videoViewTrackBarOnChanged(ITUWidget* widget, char* param)
{
	printf("videoViewTrackBarOnChanged: %s\r\n", param);
	int s = 0;
    int totaltime = 0;
	...
	if(videoPlayerIsPlaying)
	{
		s = videoViewTrackBar->value * totaltime /100;
		printf("seek to %d s\r\n", s);
		mtal_pb_seekto(s);
#if 0
		temp_sec_time = totaltime - s;
		time_buf[0] = '0';
...
bool VideoViewOnEnter(ITUWidget* widget, char* param)
{
    ...
    if (!videoPlayerStorageSprite)
    {
        ...
        videoViewTrackBar = ituSceneFindWidget(&theScene, "videoViewTrackBar");//my.wei add
        assert(videoViewTrackBar);
    }
    ...
    ituWidgetSetVisible(videoViewProgressBar, true);
	ituTrackBarSetValue(videoViewTrackBar, 0);//my.wei add
    videoViewLastTick = itpGetTickCount();
