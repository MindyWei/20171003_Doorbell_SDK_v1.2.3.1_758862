...
static void EventHandler(MJPEG_PLAYER_EVENT nEventID, void *arg)
{
    switch(nEventID)
    {
        case MJPEG_PLAYER_EVENT_EOF:
            printf("File EOF\n");
            videoPlayerIsFileEOF = true;
            break;
        case MJPEG_PLAYER_EVENT_OPEN_FILE_FAIL:
...
#ifdef CFG_VIDEO_ENABLE        
        ITE_StopMJpeg();
        ITE_PlayMJpeg(x, y, width, height, AudioGetVolume(), filepath, EventHandler);
#endif    
