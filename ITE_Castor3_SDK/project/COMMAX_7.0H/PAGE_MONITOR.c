...
/*
	if (rec_start_time)	//更新录像时间
	{
		uint32_t tick = SDL_GetTicks();
		uint32_t diff = tick - rec_start_time;
		if (diff >= 1000)
		{
			rec_start_time = tick;
			char str[6] = {'\0'};
			int min,sec;
			temp_rec_time--;
			min = temp_rec_time/60;
			str[0] = '0';
			str[1] = '0'+min;
			str[2] = ':';
			sec = temp_rec_time%60;
			if(sec < 10)
			{
				str[3] = '0';
				str[4] = '0'+sec;
			...
			ituTextSetString(MON_TEXT_REC_TIME, str);
		}
	}
*/
	...
	if(cur_page == page_motion)			//移动侦测状态下  时间检测
	{
		if(theConfig.mdtime)
		{	
			uint32_t tick_md = SDL_GetTicks();
			uint32_t diff_md = tick_md - md_start_time;
			if (diff_md >= 1000)
			{
...
{
	user_snap(2);
#if 0 //my.wei add for memory debug
	printf("monitor_rec_end: \r\n");
	ithEnterCritical();
	dbg_heap_dump("");
	ithExitCritical();
#endif
