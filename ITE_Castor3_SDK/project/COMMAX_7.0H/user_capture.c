    ...
    void)
{
	...
	if(++cnt > 3)
	{
		...
		motion_info.motion_threshold = (theConfig.mdsensitive + 1)*20;		//¼ì²âãÐÖµ  ³¬¹ý--ËãÒÆ¶¯
		motion_info.motion_sensitivity = 20;								//Êý¾Ý·Ö¿é
		motion_info.motion_start_x = 0;									//¼ì²âÇøÓò
		motion_info.motion_end_x = 1280;
		motion_info.motion_start_y = 0;
		motion_info.motion_end_y = 720;
		motion_info.motion_mask_enable = false;							//²»¼ì²éÇøÓò
