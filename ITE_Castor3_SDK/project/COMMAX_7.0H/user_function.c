...
void user_cam_detect()
{
	...
	pthread_attr_init(&cam_det_p);
	pthread_attr_setdetachstate(&cam_det_pat, PTHREAD_CREATE_DETACHED);
	pthread_create(&cam_det_p,NULL, _user_cam_detect, NULL);
	v6502_init();
}
...
user_time_init()
{
	...
	if(get_rtc_reset() || theConfig.firstboot)
	{
		printf("rtc_reset------------------------------------->\n");
		struct timeval tv;
...
void user_format_init()
{
	...
	pthread_attr_init(&format_det_p);
	pthread_attr_setdetachstate(&format_det_pat, PTHREAD_CREATE_DETACHED);
	pthread_create(&format_det_p,NULL, user_format_sd, NULL);
}
...
void user_delete_init()
{
	...
	pthread_attr_init(&delete_det_p);
	pthread_attr_setdetachstate(&delete_det_pat, PTHREAD_CREATE_DETACHED);
	pthread_create(&delete_det_p,NULL, user_delete_file, NULL);
}
...
void user_auto_init()
{
	...
	pthread_attr_init(&auto_det_p);
	pthread_attr_setdetachstate(&auto_det_pat, PTHREAD_CREATE_DETACHED);
	pthread_create(&auto_det_p,NULL, _user_auto_init, NULL);
}
