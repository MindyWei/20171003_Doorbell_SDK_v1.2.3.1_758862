...
#define CDV_704MA
#if defined(CDV_704MA)  //for CDV-704MA
...
void  user_get_voice_total_time(int nTagSize,uint32_t nFileSize,void *fp)
{
	...
	unsigned char  cur_mpeg;	//�汾
	unsigned char  cur_layer;		//��	
	unsigned char  cur_bit_hz;	//λ��
    ...
    SMTK_AUDIO_FILE_INFO *ptFileInfo)
{
    ...
    if (PalMemcmp("ID3", gtTag.pTitle, SMTK_AUDIO_MP3_ID3_V2_3_HEADER_LENGTH) == 0)
    {
        ...
        //nTagSize += 10;
		//WANG 
		user_get_voice_total_time(nTagSize ,nFileSize,audioMgr.ptNetwork.pHandle);
        LOG_DEBUG
