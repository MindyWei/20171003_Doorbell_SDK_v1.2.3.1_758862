...
static uint32_t        IIC1_GPIO_SDA      = 86;//25;//my.wei modify, 86 for TARGET_S and CDV-704MA
static uint32_t        IIC1_GPIO_SCL      = 87;//26;//my.wei modify, 87 for TARGET_S and CDV-704MA
    ...
    uint32_t     delay)
{
    ...
    if (IicDevices[port].refCount == 1)
    {
		if(port == IIC_PORT_0)
            ...
            ithWriteRegMaskA(ITH_GPIO_BASE + ITH_GPIO6_MODE_REG, (0x2 << 12 | 0x2 << 14), (0x3 << 12 | 0x3 << 14));//my.wei modify for TARGET_S and CDV-704MA
	if(IicDevices[port].ioSCLK == IIC1_GPIO_SCL)
	{
		ithGpioCtrlEnable(IIC1_GPIO_SCL, ITH_GPIO_PULL_ENABLE);
		ithGpioCtrlEnable(IIC1_GPIO_SCL, ITH_GPIO_PULL_UP);
	}
	else
	{
		ithGpioCtrlEnable(IIC1_GPIO_SDA, ITH_GPIO_PULL_ENABLE);
		ithGpioCtrlEnable(IIC1_GPIO_SDA, ITH_GPIO_PULL_UP);
	}
#elif (defined CFG_IIC1_GPIO_CONFIG_2)
    #if(defined CFG_CHIP_PKG_IT9856)
